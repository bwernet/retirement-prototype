...
const cssUrl = 'https://fonts.googleapis.com/css2?family=Figtree:ital,wght@0,300..900;1,300..900&display=swap';
...
const WANT = [
  ...
  { fam: 'Figtree', style: 'normal', weight: 800 },
  { fam: 'Figtree', style: 'italic', weight: 400 },
];
...
for (const w of WANT) {
  const candidates = blocks.filter(b => {
    if (!b.includes(`font-family: '${w.fam}'`)) return false;
    if (!b.includes(`font-style: ${w.style}`)) return false;
    // Check if block declares the weight (either as exact value or within a range)
    const weightMatch = b.match(/font-weight:\s*([\d.]+(?:\s+[\d.]+)?)/);
    if (!weightMatch) return false;
    const weights = weightMatch[1].split(/\s+/).map(Number);
    if (weights.length === 1) {
      // Exact weight
      return weights[0] === w.weight;
    } else if (weights.length === 2) {
      // Range: check if weight is within range
      return w.weight >= weights[0] && w.weight <= weights[1];
    }
    return false;
  });
...
fs.writeFileSync('bow-fonts.css', out);
console.log(`Wrote bow-fonts.css (${(out.length / 1024).toFixed(0)} KB)`);
